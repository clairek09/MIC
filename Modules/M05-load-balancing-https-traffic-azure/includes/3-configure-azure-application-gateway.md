...
( ) Autoscaling  {{With autoscaling enabled, the Application Gateway scales up or down based on application traffic requirements.}} 
( ) Manual scaling  {{With Manual scaling enabled, the gateway won't autoscale. If there is more traffic than the Application Gateway can handle, it could result in traffic loss.}} 
( )  Health probes  {{Health probes monitor the health of all resources in the back-end pool and automatically removes any resource considered unhealthy from the pool.}}

## Multiple Choice 

What is a listener?
( ) A listener is an entity that checks for incoming connection requests.  {{A listener is a logical entity that checks for incoming connection requests by using the port, protocol, host, and IP address.}} 
( ) A listener is an entity that routes traffic based on basic or path-based rules.  {{A routing rule is an entity that routes traffic based on basic or path-based rules.}} 
( )  A listener is a collection of servers that respond to requests.  {{A backend pool}}
