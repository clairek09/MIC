...
![Screenshot of the Add Connection page in the Azure portal. TestVNet4GW is selected as the second virtual network gateway. The Shared key is abc123.](../media/gateway-connections.png)
...
( ) PolicyBased and RouteBased. {{VPNs can be RouteBased or PolicyBased.}}

( ) PolicyBased and static. {{VPNs can be PolicyBased. Static is the name used for PolicyBased VPNs in the classic deployment model.}}

( ) RouteBased and dynamic. {{VPNs can be RouteBased. Dynamic is the name used for RouteBased VPNs in the classic deployment model.}} 
...
( ) Active-standby {{Every Azure VPN gateway consists of two instances in an active-standby configuration.}}

( ) Active-active {{You can create an Azure VPN gateway in an active-active configuration, but this is not the defuault.}}

( ) Dual-redundancy {{This is the most reliable option, combining the active-active gateways on both your network and Azure, but it is not the default.}}